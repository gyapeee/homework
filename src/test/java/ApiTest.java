...
public class ApiTest {
  ...
  public void getUsersAndVerifyAtInFirstEmail() throws IOException {
    ...
    Assert.assertTrue(users.get(0)
                           .getEmail()
                           .contains("@"), "@ symbol is missing from the first user's email ");
  }

  private static List<User> parseFromJson(Response response) throws IOException {
    return new Gson().fromJson(Objects.requireNonNull(response.body())
                                      .string(), new TypeToken<List<User>>() {
    }.getType());
  }
